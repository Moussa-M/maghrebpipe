...
export class ContactPage implements OnInit {
  ...
  showOfficeAddress: Boolean = true;
  google;
  constructor(private translateService: TranslateService, private popoverController: PopoverController, private platform: Platform) { }

  ngOnInit() {
    GoogleMapsLoader.load().then((mapsApi) => {
      this.google = mapsApi;
      this.loadMap(this.google);
    });

  }

  loadMap(google) {
    let office_latlng = { lat: 36.720624, lng: 3.185494 };
    let factory_latlng = { lat: 35.691242, lng: 4.52894 };
    ...
    var office_m = new google.maps.Marker({
      position: office_latlng,
      map: map,
      icon: this.svgOfficePin('#F26A35'),
      title: 'MAGHREB PIPE SIÈGE SOCIALE'
    });

    var factory_m = new google.maps.Marker({
      position: factory_latlng,
      map: map,
      icon: this.svgFactoryPin('#F26A35'),
      title: 'MAGHREB PIPE BUREAU'
    });

    office_m.addListener('click', ()=> {
      this.showOfficeAddress=true;
      map.setZoom(12);
      map.panTo(office_m.getPosition());
    });
 factory_m.addListener('click', ()=> {
      this.showOfficeAddress=false;
      map.setZoom(12);
      map.panTo(factory_m.getPosition());
     
    });

    // map.addListener('click',()=>{map.setZoom(7)});
